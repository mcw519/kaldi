...
from egs_dataloader_multi import get_egs_dataloader
from libs.nnet3.train.dropout_schedule import _get_dropout_proportions
from model_2output_lid_discrimator import get_chain_model
from options import get_args
from discriminator import supervision2langid


def get_objf(batch, model, device, criterion, opts, den_graph, training, optimizer=None, dropout=0.):
    feature, supervision, oid = batch
    assert feature.ndim == 3

    if oid == int(1):
        for name, p in model.named_parameters():
            if name.startswith('prefinal_2'):
    ...
    ##### First time update ASR task
    # at this point, feature is [N, T, C]
    feature = feature.to(device)
    if training:
        nnet_output, xent_output, _ = model(feature, dropout=dropout, lang=oid)
    else:
        with torch.no_grad():
            nnet_output, xent_output, lang_id = model(feature, lang=None)
    ...
    if training:
        ...
        _, _, lang_id = model(feature, dropout=dropout, lang='lid')
        target = supervision2langid(supervision)
        # target length is [ frames_per_sequence * num_sequences ]
        target = target.reshape(N, T)
        target = target.to(lang_id.device)
        
        #lang_id = lang_id.contiguous().view(-1, lang_id.shape[-1])
        ## lang_id is [ 6400, 3]

        lang_loss = 0.
        for i in range(N):
            _loss = torch.nn.NLLLoss(weight=torch.Tensor([0.5, 0.5, 1.0]).to(device))(lang_id[i], target[i])
            lang_loss += _loss
    ...
    total_objf = objf_l2_term_weight[0].item() + lang_loss
    total_weight = objf_l2_term_weight[2].item()
    total_frames = nnet_output.shape[0]



    return total_objf, total_weight, total_frames, lang_loss
...
def get_validation_objf(dataloader, model, device, criterion, opts, den_graph):
    ...
    for batch_idx, (pseudo_epoch, batch) in enumerate(dataloader):
        objf, weight, frames, lang_loss = get_objf(
    ...
    return total_objf, total_weight, total_frames, total_lang_loss
...
def train_one_epoch(dataloader, valid_dataloader, model, device, optimizer, criterion, 
    ...
    for batch_idx, (pseudo_epoch, batch) in enumerate(dataloader):
        ...
        curr_batch_objf, curr_batch_weight, curr_batch_frames, curr_batch_lang_objf = get_objf(
            batch, model, device, criterion, opts, den_graph, True, optimizer, dropout=dropout)

        total_objf += curr_batch_objf
        total_weight += curr_batch_weight
        total_frames += curr_batch_frames
        total_lang_objf += curr_batch_lang_objf
        ...
        if valid_dataloader and batch_idx % 1000 == 0:
            total_valid_objf, total_valid_weight, total_valid_frames, total_lang_loss = get_validation_objf(
            ...
            if tf_writer:
                ...
                tf_writer.add_scalar('train/global_valid_lang_objf',
                                 total_lang_loss,
                                 pseudo_epoch + current_epoch * len(dataloader))
        ...
        if (rank is None or rank == 0) and batch_idx % 100 == 0 and tf_writer is not None:
            ...
            tf_writer.add_scalar(
                'train/current_batch_lang_objf',
                curr_batch_lang_objf ,
                pseudo_epoch + current_epoch * len(dataloader))
