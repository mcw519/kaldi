...
def get_egs_dataloader(egs_dir_or_scp,
    ...
    '''
    # oid is multi output layer's key
...
class NnetChainExampleScpDataset(Dataset):
    ...
    def __init__(self, egs_dir_or_scp):
        ...
        inside it.
        TODO: support multi-lang training
        1. read output key ark file.
        '''
...
class NnetChainExampleCollateFunc:
    ...
    def __call__(self, eg):
        ...
        eg.outputs[0].name is the key for which output layer
        '''
        assert eg.inputs[0].name == 'input'
        assert len(eg.outputs) == 1
        #assert eg.outputs[0].name == 'output'

        if eg.outputs[0].name == 'output-default_1':
            oid = int(1)
        elif eg.outputs[0].name == 'output-default_2':
            oid = int(2)
        else:
            oid = None
        ...
        return torch_feat, supervision, oid


def _test_nnet_chain_example_dataloader():
    scp_dir = '/nfs/TPAICSASR03/meng/multi_lang/exp/mandarin_ls360_train_2output/egs_chain2_for_training/cegs.1.scp'
...
def _test_dataloader_iter(scp_dir_or_file):
    ...
    for i in range(2):
        ...
        for pseudo_epoch, batch in dataloader:
            ...
            print('feat shape is {}, oid is {}, supervision weight is {}'.format(feature.shape, oid, supervision.weight))
