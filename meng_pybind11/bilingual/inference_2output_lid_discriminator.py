...
def main():
    ...
    specifier_lid = 'ark,scp:{filename}.ark,{filename}.scp'.format(
        filename=os.path.join(args.dir, 'lid'))
    ...
    writer_lid = Writer(specifier_lid)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            nnet_output, _, lang_id = model(padded_feat, lang=None)
        ...
        for i in range(num):
            ...
            value = value.cpu()
            result_id = lang_id[first:first + target_len, :, :].split(1, 0)
            value_id = torch.cat(result_id, dim=1)[0, :output_len, :]
            value_id = value_id.cpu()
            ...
            m_lid = kaldi.SubMatrixFromDLPack(to_dlpack(value_id))
            m_lid = Matrix(m_lid)
            writer_lid.Write(key, m_lid)
