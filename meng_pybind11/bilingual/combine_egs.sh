...
lang2weight=1.0,1.0            # array of weights one per input languge to scale example's output
...
mkdir -p $megs_dir
shift 1
args=("$@") # a list args[0] is egs_1, args[1] is egs_2
...
#lang_list=$(for i in `seq 0 $[$num_langs-1]`; do awk '/^lang/' ${args[$i]}/info.txt | awk '{print $2}'; done)
for i in `seq 0 $[$num_langs-1]`; do
  lang_list[$i]=`awk '/^lang/' ${args[$i]}/info.txt | awk '{print $2}'`
done

for i in `seq 0 $[$num_langs-1]`; do
  echo "langs ${lang_list[$i]}" >> $megs_dir/info.txt #########
done
...
if [ $stage -le 0 ]; then
    ...
    for lang in $(seq 0 $[$num_langs-1]);do
        lang_name=${lang_list[$lang]}
        if [ ! -d $dir/temp_${lang_name} ]; then
          mkdir $dir/temp_${lang_name}
        fi
