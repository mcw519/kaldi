...
acwt=1.0 #0.1  # Just a default value, used for adaptation and beam-pruning..
post_decode_acwt=10.0 #1.0  # can be used in 'chain' systems to scale acoustics by 10 so the
                      # regular scoring script works.
cmd=run.pl
beam=15.0
frames_per_chunk=150 #50
max_active=7000
min_active=200
ivector_scale=1.0
lattice_beam=6.0 #8.0  # Beam we use in lattice generation. We can reduce this if 
...
ctm_frame_shift=0.01
if [ -f $srcdir/frame_subsampling_factor ]; then
  # e.g. for 'chain' systems
  frame_subsampling_opt="--frame-subsampling-factor=$(cat $srcdir/frame_subsampling_factor)"
  ctm_frame_shift=0.03
fi
...
if [ $stage -le 1 ]; then
  ...
  wd_wspecifier="ark,t:| int2sym.pl -f 2- $graphdir/words.txt - > $dir/wd.JOB"
  ali_wspecifier="ark,t:$dir/align.JOB.ark"
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
    nnet3-latgen-faster$thread_string $ivector_opts $frame_subsampling_opt \
     ...
     "$HCLG" "$feats" "$lat_wspecifier" "$wd_wspecifier" "$ali_wspecifier" || exit 1;

  cat $dir/wd.* | sort > $dir/wd.txt
  #python3 confidence_island/src/confidence_island.py $data/text $dir/wd.txt
fi

if [ $stage -le 2 ]; then
  echo "$0 Find islands"
  #steps/get_ctm.sh --cmd $cmd --frame-shift $ctm_frame_shift $data $graphdir $dir
  steps/get_ctm_fast.sh --cmd $cmd --frame-shift $ctm_frame_shift $data $graphdir $dir $dir/ctm_10
  name=`basename $data`
  #python3 confidence_island/src/confidence_island.py $data/text $dir/wd.txt $dir/score_10/$name.ctm
  python3 src/confidence_island.py $data/text $dir/wd.txt $dir/ctm_10/ctm
fi
