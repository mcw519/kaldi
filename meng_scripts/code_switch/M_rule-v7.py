...
for i in range(0, len(line)):
	...
	# rule 1, check "AH0 M" in the end or not.
	if (W_P[len(W_P)-1] == "M" and W_P[len(W_P)-2] == "AH0"):
		W_P[len(W_P)-1] = "ern0"
		W_P[len(W_P)-2] = "NULL" # NULL is a removed symbol.
	elif (W_P[len(W_P)-1] == "M" and W_P[len(W_P)-2] == "AH1"):
                W_P[len(W_P)-1] = "ern1"
                W_P[len(W_P)-2] = "NULL"
	elif (W_P[len(W_P)-1] == "M" and W_P[len(W_P)-2] == "AH3"):
		W_P[len(W_P)-1] = "ern3"
                W_P[len(W_P)-2] = "NULL"

	# rule 3, M not present in front of vowel.
	for j in range(1, len(W_P)-1): # check until last 2 phoneme.
		if W_P[j] == "M" and W_P[j+1] not in Vowel:
			if W_P[j-1] == "AH0":
				W_P[j-1] = "NULL" # AH0 M merge to ern0.
				W_P[j] = "ern0"
			elif W_P[j-1] == "AH1":
				W_P[j-1] = "NULL" # AH1 M merge to ern1.
                                W_P[j] = "ern1"
			elif W_P[j-1] == "AH3":
                                W_P[j-1] = "NULL" # AH3 M merge to ern3.
                                W_P[j] = "ern3"
			else:
				W_P[j] = "ern1" # Voiced Consonants.
